...
class UploadsController extends Controller
{
    ...
    public function upload(Request $request)
    {
        if ($request->isMethod('POST')) {
            ...
            $file_s = time() . '_' . uniqid() . '.' . $extension;
            $file_name = base_path() . '/storage/app/uploads/' . $file_s;
            ...
            $bucket = 'mioji.test';
            $client = new Ks3Client('K0fJOlnF++fpBboIJX3x', 'qaU28hp560VjOjjTXYwKb2OmIPspU1oRZgYLbW+9', 'kss.ksyun.com');
            ...
            if (isset($re['ETag'])) {
                file_exists($file_name) ? unlink($file_name) : false;  //金山云上传成功，删除本地图片

                $cdn = 'cdn.mioji.com';
                $data = [
                    'url' => 'http://' . $bucket . '.' . $cdn . '/' . $file_s,
                ];
